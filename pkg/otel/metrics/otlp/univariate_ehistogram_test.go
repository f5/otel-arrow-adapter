...
func TestExponentialHistograms(t *testing.T) {
	...
	s := arrow.NewSchema([]arrow.Field{
		{Name: "ehistograms", Type: arrow2.UnivariateEHistogramDT, Metadata: schema.Metadata(schema.Optional)},
	}, nil)
	...
	for {
		...
		b := arrow2.UnivariateEHistogramBuilderFrom(rBuilder.StructBuilder("ehistograms"))
		for i := 0; i < maxIter; i++ {
