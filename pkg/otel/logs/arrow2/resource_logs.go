...
func NewResourceLogsBuilder(pool memory.Allocator, dictConfig *cfg.DictionaryConfig) *ResourceLogsBuilder {
	s := arrow.NewSchema([]arrow.Field{
		{Name: constants.ResourceLogs, Type: ResourceLogsDT, Metadata: schema.Metadata(schema.Optional)},
	}, nil)
	rBuilder := builder.NewRecordBuilderExt(pool, s, dictConfig)

	return ResourceLogsBuilderFrom(rBuilder.StructBuilder(constants.ResourceLogs))
}
