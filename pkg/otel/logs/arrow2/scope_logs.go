...
func NewScopeLogsBuilder(pool memory.Allocator, dictConfig *cfg.DictionaryConfig) *ScopeLogsBuilder {
	s := arrow.NewSchema([]arrow.Field{
		{Name: constants.ScopeLogs, Type: ScopeLogsDT, Metadata: schema.Metadata(schema.Optional)},
	}, nil)
	rBuilder := builder.NewRecordBuilderExt(pool, s, dictConfig)

	return ScopeLogsBuilderFrom(rBuilder.StructBuilder(constants.ScopeLogs))
}
